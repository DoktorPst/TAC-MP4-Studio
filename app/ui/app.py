"""TAC MP4 Studio — Update 2

Nouveautés :
  1. Waveform globale cliquable sous la preview (navigation dans le son)
  2. Champs Artiste + Titre séparés (rendu différent sur la vidéo)
  3. Raccourcis clavier : Espace (play/pause), R (recharger), Échap (accueil)
"""
...
WAVEFORM_H = 56   # hauteur canvas waveform en px
...
class App(ctk.CTk if not _DND_AVAILABLE else TkinterDnD.Tk):
    ...
    def __init__(self) -> None:
        ...
        self.artist_text      = tk.StringVar(value=settings.get("artist_text", ""))  # Update 2
        ...
        # ── Preview state ──────────────────────────────────────────────────────
        self.preview_is_vertical: bool = False
        ...
        # ── Update 2 : waveform state ──────────────────────────────────────────
        self.waveform_data: np.ndarray | None = None  # array float32 [0-1]
        self.audio_total_duration: float = 0.0
        self._waveform_canvas: tk.Canvas | None = None
        self._waveform_cursor_id: int | None = None

        self._build_ui()
        self.protocol("WM_DELETE_WINDOW", self._on_close)

        # ── Update 2 : raccourcis clavier ─────────────────────────────────────
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # RACCOURCIS CLAVIER (Update 2 — Feature 3)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    def _kb_escape(self):
        """Retour accueil depuis l'éditeur uniquement."""
        if hasattr(self, "preview_label") and self.preview_label:
            self.show_home()

    # ══════════════════════════════════════════════════════════════════════════
    # WAVEFORM (Update 2 — Feature 1)
    # ══════════════════════════════════════════════════════════════════════════

    def _load_waveform(self, audio_path: str):
        """Charge la forme d'onde en arrière-plan (basse résolution)."""
        def worker():
            try:
                # SR bas pour aller vite : 4000 Hz suffit pour la waveform
                y, sr = librosa.load(audio_path, sr=4000, mono=True)
                duration = len(y) / sr
                # RMS par blocs de ~80ms → ~200 points pour 16s, ~2250 pour 3min
    ...
    def _draw_waveform(self, cursor_t: float | None = None):
        """Redessine le canvas waveform. cursor_t = position en secondes."""
        ...
        for i, val in enumerate(data):
            ...
            c.create_rectangle(x, center - amp, x + bar_w - 0.5,
                                center + amp, fill=color, outline="")

        # Zone preview en surbrillance
        ...
        if dur > 0:
            c.create_text(4, h - 4, text=f"0:00", anchor="sw",
                          fill=MUTED, font=("Segoe UI", 7))
            mid_s = int(dur / 2)
            c.create_text(w / 2, h - 4, text=f"{mid_s//60}:{mid_s%60:02d}",
                          anchor="s", fill=MUTED, font=("Segoe UI", 7))
            tot = int(dur)
            c.create_text(w - 4, h - 4, text=f"{tot//60}:{tot%60:02d}",
                          anchor="se", fill=MUTED, font=("Segoe UI", 7))

    def _on_waveform_click(self, event):
        """Clic sur la waveform → set preview_start et recharge."""
        c = self._waveform_canvas
        ...
        if w <= 0:
            return
        t = (event.x / w) * self.audio_total_duration
        t = max(0.0, min(t, self.audio_total_duration - 1))
        self.preview_start.set(f"{t:.1f}")
        self._draw_waveform()
        self._prepare_preview()

    def _tick_waveform_cursor(self):
        """Met à jour le curseur waveform pendant la lecture."""
        ...
        if self.audio_playing and self.preview_started_at is not None:
            try:
                ...
                t0 = 0.0
            current_t = t0 + (time.time() - self.preview_started_at)
            self._draw_waveform(cursor_t=current_t)
    ...
    def _check_ffmpeg(self):
        ...
        _btn(banner, "✕", lambda: (banner.destroy()),
    ...
    def _build_ui(self):
        ...
        hints = "  ⌨  Espace : play/pause  ·  R : recharger  ·  Échap : accueil"
        ctk.CTkLabel(hdr, text=hints, text_color=MUTED,
                     font=FONT_MU).pack(side="left", padx=20)
    ...
    def show_editor(self):
        ...
        # ── Chargement waveform dès l'ouverture de l'éditeur ──────────────────
        if self.audio_path:
        ...
        # ── Waveform canvas (Update 2 — Feature 1) ─────────────────────────
        ...
        # Tooltip waveform
        ctk.CTkLabel(wf_frame,
                     text="Clic pour déplacer le départ preview · Zone violette = preview",
        ...
        ctk.CTkLabel(files_card, text=self._short_name(self.audio_path),
                     text_color=ACCLT, font=FONT_SM, anchor="w").pack(fill="x", padx=14, pady=(10, 2))
        ctk.CTkLabel(files_card, text=self._short_name(self.image_path),
        ...
        # Section Texte — Update 2 : Artiste + Titre séparés
        self._section_title(r, "✍  Texte")
        txt_card = _card(r)
        txt_card.pack(fill="x", pady=(0, 10), padx=4)
        tc = ctk.CTkFrame(txt_card, fg_color="transparent")
        tc.pack(fill="x", padx=10, pady=10)

        ctk.CTkLabel(tc, text="Artiste", text_color=MUTED, font=FONT_MU, anchor="w").pack(anchor="w")
        ctk.CTkEntry(tc, textvariable=self.artist_text,
        ...
        ctk.CTkLabel(tc, text="Titre", text_color=MUTED, font=FONT_MU, anchor="w").pack(anchor="w")
        ctk.CTkEntry(tc, textvariable=self.title_text,
                     placeholder_text="Titre du son  (optionnel)",
                     fg_color=SURF3, border_color=BORDER, text_color=TEXT,
                     font=FONT_SM).pack(fill="x", pady=(2, 8))

        # Prévisualisation du rendu texte
        self._text_preview_lbl = ctk.CTkLabel(tc, text="", text_color=ACCLT,
        ...
        _sep(tc)
        self._slider_row(tc, "Position X", self.text_x, 0.05, 0.95)
        self._slider_row(tc, "Position Y", self.text_y, 0.15, 0.92)
    ...
    def _prepare_preview(self):
        ...
        def worker():
            try:
                ...
                self.after(0, lambda: self._draw_waveform())
    ...
    def _reload_visuals_only(self):
        ...
        try:
            ...
            bg, cov = load_cover_image(s.image_path, s.background_blur,
                                        s.image_zoom, s.output_width, s.output_height)
            self.preview_bg        = bg
            self.preview_cover     = cov
            self.preview_particles = []
            self.preview_smoke     = []
    ...
    def _tick_preview(self):
        ...
        frame, self.preview_particles, self.preview_smoke, self.preview_smoothed = render_frame(
            ...
            metrics, self.preview_smoothed, settings,
        )
    ...
    def _play_preview_audio(self):
        ...
        try:
            ...
            self.preview_index = 0
            self._set_status("▶ Audio preview  (Espace pour pause)", SUCCESS)
    ...
    def _copy_assets(self, proj_dir: Path, name: str):
        ...
        shutil.copy2(a,   proj_dir / f"{name}{a.suffix.lower()}")
        shutil.copy2(img, proj_dir / f"{name}_cover{img.suffix.lower()}")
        return (str(proj_dir / f"{name}{a.suffix.lower()}"),
                str(proj_dir / f"{name}_cover{img.suffix.lower()}"))
    ...
    def _start_export(self):
        ...
        def worker():
            try:
                ...
                self.history.append({
                    "name": file_name, "folder": str(proj_dir),
                    "video": settings.output_path, "audio": settings.audio_path,
                    "image": settings.image_path, "type": mode.lower(),
